...
## For running all complaints ##

# read in
complaints = pd.read_csv("C:/Users/emercado/OneDrive - Ofsted/January 2023/Complaints analysis/cleaned_complaints.csv")

# define summariser function
def summariser_fn(filename, idno):
    singlecomplaint = filename['text'][idno]
    batch = tokenizer(singlecomplaint, truncation = True, padding = 'longest', return_tensors = "pt").to(torch_device)
    translated = model.generate(**batch)
    tgt_text = tokenizer.batch_decode(translated, skip_special_tokens = True)
...
# get no of complaints
x = len(complaints)

output = pd.DataFrame(columns=['complaint', 'complaint_id', 'complaint_text', 'summary'])
for i in range(0,x):
    test = summariser_fn(complaints, i)
    data = pd.DataFrame([[complaints['complaint'][i], complaints['complaint_id'][i], complaints['text'][i], test]], 
                        columns = ['complaint', 'complaint_id', 'complaint_text', 'summary'])
